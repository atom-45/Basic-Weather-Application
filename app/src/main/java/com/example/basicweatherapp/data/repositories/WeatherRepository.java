...
public class WeatherRepository {
    ...
    private final CachedForecastDAO cachedForecastDAO;
    private final Gson gson = new Gson();

    @Inject
    public WeatherRepository(APIService apiService, Application application){
        this.apiService = apiService;
        this.cachedForecastDAO = WeatherDatabase.getInstance(application).cachedForecastDAO();
    }
    ...
    public Observable<ForecastResponse> getCachedForecast() {
        return cachedForecastDAO.getCachedForecast()
                .map(cached -> gson.fromJson(cached.getJsonData(), ForecastResponse.class));
    }

    public Completable cacheForecast(ForecastResponse response) {
        return cachedForecastDAO.insertCachedForecast(new CachedForecast(gson.toJson(response)));
    }
